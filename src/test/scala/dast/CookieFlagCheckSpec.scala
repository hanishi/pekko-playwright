...
class CookieFlagCheckSpec extends AnyWordSpec with Matchers {
  ...
  private def cookie(
    ...
    Cookie(name, "v", "example.com", "/", httpOnly, secure, sameSite)
  ...
  "CookieFlagCheck" should {
    ...
    "report nothing for a fully-secured cookie" in {
      CookieFlagCheck.check(
        snap(cookie("sessionid", httpOnly = true, secure = true, sameSite = Some("Lax"))),
      ) shouldBe empty
    }

    "flag all three missing attributes on a session cookie" in {
      val findings = CookieFlagCheck.check(
        snap(cookie("sessionid", httpOnly = false, secure = false, sameSite = None)),
      )
    ...
    "rate a missing HttpOnly higher on a session cookie than on a plain one" in {
      def httpOnlySeverity(name: String): Severity =
        CookieFlagCheck
          .check(snap(cookie(name, httpOnly = false, secure = true, sameSite = Some("Lax"))))
          .find(_.evidence.contains("HttpOnly"))
          .get
          .severity
