...
class PayloadLibrarySpec extends AnyWordSpec with Matchers {
  ...
  "PayloadLibrary" should {
    ...
    "expose every payload's id via ids" in {
      PayloadLibrary.ids should contain allOf ("img-onerror", "svg-onload", "script-tag")
      PayloadLibrary.ids.foreach(id => PayloadLibrary.get(id).map(_.id) shouldBe Some(id))
    }
  ...
  "escapeJsString" should {
    ...
    "prevent a </script> breakout" in {
      val out = PayloadLibrary.escapeJsString("</script><img src=x onerror=alert(1)>")
