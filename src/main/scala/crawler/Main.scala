...
/** Runnable one-shot crawler.
  ...
  * args, spawns a [[Crawler]], writes each scraped page to a two-column
  * `url,text` CSV, and exits when the crawl finishes. `Crawler` stops itself
  * once nothing is in flight; the guardian watches it and stops on
  * `Terminated`, which terminates the ActorSystem (and the forked JVM) —
  * closing the browser pool and the CSV writer on the way out.
  *
  * Usage:
  ...
  * `CRAWLER_CONFIG` < CLI args (seed URL, max depth, output path).
  * Output path resolution: arg 2, else `crawler.output-csv`, else `crawler-output.csv`.
  */
object Main:
  ...
  private def loadConfig(): Config =
    ...
    sys.env.get("CRAWLER_CONFIG").map(_.trim).filter(_.nonEmpty) match
      ...
      case Some(path) =>
        val f = new File(path)
        if !f.isFile then
          log.warn("CRAWLER_CONFIG={} is not a readable file; using bundled defaults", path)
          bundled
        else
  ...
  private def resolveOutput(config: Config, args: List[String]): Path =
    ...
    val fromCfg = Option.when(config.hasPath("crawler.output-csv"))(config.getString("crawler.output-csv"))
    Paths.get(fromArg.orElse(fromCfg).getOrElse("crawler-output.csv"))

  /** Quote a field per RFC 4180: wrap in double quotes and double any
    * embedded double quotes. Always quoting is safe since page text routinely
    * contains commas, quotes, and newlines. */
  private def csv(field: String): String =
    "\"" + field.replace("\"", "\"\"") + "\""

  private def guardian(config: CrawlerConfig, outputPath: Path): Behavior[PageContent] =
    Behaviors.setup { ctx =>
      ctx.log.info(
      ...
      val writer: BufferedWriter = Files.newBufferedWriter(outputPath, StandardCharsets.UTF_8)
      ...
      Behaviors
        ...
        .receiveSignal { case (sctx, Terminated(_)) =>
          writer.close()
          sctx.log.info("Crawl complete — wrote {} rows to {}", rows, outputPath.toAbsolutePath)
          Behaviors.stopped
        }
    }
