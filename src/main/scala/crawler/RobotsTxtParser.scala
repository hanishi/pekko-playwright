...
class RobotsTxtParser(
    val disallowedPaths: Seq[String],
    val allowedPaths: Seq[String],
):

  /** Check if a URL is allowed by robots.txt rules. Allow rules take precedence
    * over disallow when the allow path is more specific.
    */
  ...
  private def matchesRule(path: String, rule: String): Boolean =
    if (rule.endsWith("*")) path.startsWith(rule.dropRight(1))
    else if (rule.endsWith("$")) path == rule.dropRight(1)
    else path.startsWith(rule)
