...
/** Turns model output (a JSON string, or equivalently the `input` of an
  ...
  * into an executable decision. Extra fields a model might emit (a stray `code`
  * or `script`) are simply ignored, so they cannot reach execution. Callers
  * fail closed on `Left` (default to a no-op, never to a guess).
  */
