...
/** One model-chosen navigation move (closed ADT, per instruction.md §0.2). The
  * model selects an indexed element on the current page; it never invents a
  * URL. Parsing is the boundary and fails closed to [[Done]].
  */
enum NavStep:
...
object NavStep:
  ...
  def parse(input: ujson.Value): NavStep =
    input.objOpt.flatMap(_.get("action")).flatMap(_.strOpt).map(_.toLowerCase)
    match
  ...
  def render(
    ...
    val fs = forms.zipWithIndex.map { (f, i) =>
      ...
      s"  form[$i] method=${f.method} action=${f.action} fields=[$fields] submit='${f.submitText}'"
    }.mkString("\n")
    ...
    s"""Current page: $url
       ...
       |Already reached: ${if history.isEmpty then "(nothing yet)"
      else history.mkString(", ")}
       |
  ...
  private def intField(v: ujson.Value, name: String): Option[Int] = v.objOpt
    .flatMap(_.get(name)).flatMap(x =>
      x.numOpt.map(_.toInt).orElse(x.strOpt.flatMap(_.toIntOption)),
    )
