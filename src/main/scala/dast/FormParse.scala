...
object FormParse:
    ...
    .findAllMatchIn(html).map { m =>
      ...
      val submitText = (
        buttonRe.findAllMatchIn(body).map(_.group(1)) ++
          ...
          }.flatMap(im => attr(im.group(2), "value"))
      ).mkString(" ").replaceAll("(?is)<[^>]+>", " ").trim
  ...
  private def resolve(base: String, href: String): Option[String] = Try(
    new java.net.URI(base).resolve(href).toString,
  ).toOption
