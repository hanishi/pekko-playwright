...
/** Runnable site scanner: crawls in-scope URLs from a seed and scans each,
  ...
  * sink-scan is opt-in via `DAST_AUTHORIZED_HOSTS` and needs `ANTHROPIC_API_KEY`
  * for the analyzer (fails closed otherwise). `DAST_MAX_PAGES` (default 20),
  * `DAST_MAX_DEPTH` (default 2), `DAST_NAV_TIMEOUT_MS` (default 30000) tune the
  * crawl. Wiring around tested components; exercised only by a live run against
  * a consenting target.
  *
  * Usage:
  *   sbt "runMain dast.scan.SiteScannerMain https://example.com/"
  */
object SiteScannerMain:
  ...
  private def authorization: Authorization =
    sys.env.get("DAST_AUTHORIZED_HOSTS").map(_.trim).filter(_.nonEmpty) match
      case Some(hosts) => Authorization.active(hosts.split(",").map(_.trim).toIndexedSeq*)
      case None        => Authorization.ObserveOnly

  private def guardian(seed: String, auth: Authorization): Behavior[SiteScanComplete] =
    Behaviors.setup { ctx =>
      ...
      ctx.log.info(
        ...
        seed,
        if auth.allowActive then auth.authorizedHosts.mkString(",") else "observe-only",
