...
/** Runnable DAST scanner. Scans one target and prints a findings report.
  ...
  *
  * Usage:
  *   sbt "runMain dast.scan.ScannerMain https://example.com/path?q=1"
  *   DAST_AUTHORIZED_HOSTS=example.com ANTHROPIC_API_KEY=sk-... \
  *     sbt "runMain dast.scan.ScannerMain https://example.com/path?q=1"
  */
object ScannerMain:
  ...
  private def guardian(target: String, auth: Authorization): Behavior[ScanComplete] =
    Behaviors.setup { ctx =>
      ...
      val scanner = Scanner.spawn(ctx, auth)
