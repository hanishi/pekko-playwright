...
/** Turns an [[AnalyzerContext]] into one validated [[LlmDecision]] by calling
  ...
  * authoritative boundary that rejects anything off-menu. The call fails
  * closed: a missing key, HTTP failure, non-200, missing tool_use block, or
  * parse failure all yield [[LlmDecision.Done]] (a no-op), never a guess.
  *
...
object ClaudeAnalyzer:
  ...
  def buildRequestBody(context: AnalyzerContext): ujson.Value =
    ...
    ujson.Obj(
      ...
      "messages" ->
        ujson.Arr(ujson.Obj("role" -> "user", "content" -> context.render)),
    )
  ...
  /** Map a Messages API response body to a decision, failing closed to `Done`.
    ...
    * [[DecisionParser]].
    */
  def responseToDecision(body: ujson.Value): LlmDecision = decideToolInput(body)
    .map(input => DecisionParser.parse(ujson.write(input))).flatMap(_.toOption)
    .getOrElse(LlmDecision.Done)

  private def decideToolInput(body: ujson.Value): Option[ujson.Value] = body
    .objOpt.flatMap(_.get("content")).flatMap(_.arrOpt).flatMap { blocks =>
      blocks.find { b =>
        b.objOpt.exists { o =>
          o.get("type").flatMap(_.strOpt).contains("tool_use") && o.get("name")
            .flatMap(_.strOpt).contains(DecisionTool.Name)
        }
      }
      ...
      case Some(apiKey) =>
        ...
        val request = HttpRequest(
          ...
          entity = HttpEntity(
            ...
            ujson.write(buildRequestBody(context)),
          ),
        )
