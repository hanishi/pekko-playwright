...
/** The LLM planning step for IDOR: turn an [[Observation]] of an authenticated
  ...
  * parameters; [[IdorPlan.parseProposals]] validates them and deterministic code
  * ([[dast.IdorProbe]]) confirms. Fails closed to an empty plan: a missing key,
  * HTTP error, or unparseable response yields no proposals, never a guess.
  *
...
object IdorPlanner:
  ...
  private val SystemPrompt =
    "You are the planning step of a consented IDOR (insecure direct object " +
  ...
  def buildRequestBody(obs: Observation): ujson.Value = ujson.Obj(
    ...
    "messages" ->
      ujson.Arr(ujson.Obj("role" -> "user", "content" -> obs.render)),
  )
    ...
    case Some(apiKey) =>
      ...
      Http()(system).singleRequest(request).flatMap { response =>
        if response.status.isSuccess() then Unmarshal(response.entity).to[String]
          .map { raw =>
            val ps = Try(ujson.read(raw)).toOption
              .map(responseToProposals).getOrElse(Seq.empty)
            log.info("IDOR planner proposed {} test(s) for {}", ps.size, obs.url)
            ps
          }
        else
