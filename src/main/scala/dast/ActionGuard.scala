...
/** The deterministic safety floor for LLM-driven form submission (instruction.md
  * §5 navigation-action carve-out).
  *
...
object ActionGuard:
  ...
  private val denyPatterns: Seq[String] = Seq(
    "delete", "remove", "destroy", "drop", "pay", "payment", "checkout",
    "purchase", "buy", "transfer", "withdraw", "logout", "signout", "sign-out",
    "password", "passwd", "email", "sendmail", "confirm", "cancel", "deactivate",
  ...
  private def denyHit(form: FormInfo): Option[String] =
    val haystack = (form.action + " " + form.fields.map(_._1).mkString(" ") +
